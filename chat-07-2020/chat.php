...
{
	$sql = "INSERT INTO chatlog (chatstring, note) VALUE (\"$chatstring\", '')";
...
function load_chatlog($db)
{
	...
	$sql = "SELECT chatstring, ID, DATE_FORMAT(`date`, \"%b-%d %h:%i %p\") `date` FROM (SELECT chatstring, ID, `date` FROM chatlog ORDER BY id DESC LIMIT 100) sub ORDER BY id ASC";
